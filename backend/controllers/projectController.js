...
exports.fetchProjects = async (req, res) => {
  try {
    ...
    const projectRoles = await ProjectRole.find({ userId: userId }).populate('projectId');
    
    const projectIds = projectRoles.map(role => role.projectId._id);

    const projects = await Project.find({ $or: [{ owner: userId }, { _id: { $in: projectIds } }] });
...
exports.provideAccess = async (req, res) => {
  try {
    ...
    if (!user) {
      res.status(400).json({ message: `User doesnot exist! Create user with email ${email}` });
    }
...
exports.addAllowedAccess = async (req, res) => {
  try {
    ...
    const acceptedNewRoles = newRoles.filter(role => roleTypes.includes(role));

...
const LoginForm = () => {
  ...
  return (
    <div className='flex flex-col justify-center items-center p-8 w-full tablet:w-1/2 overflow-hidden'>
      ...
      <div className='mb-6 text-2xl font-bold w-2/3 tablet:text-3xl'>Log in to your account</div>

      <label className='tablet:w-2/3 mt-6'>Email</label>
      <Input type='email' onChange={handleChange} name='email' classes='tablet:w-2/3 mt-3 border rounded shadow h-10 px-4' />

      <label className='tablet:w-2/3 mt-6'>Password</label>
      ...
      <div className='w-full flex justify-between items-center tablet:w-2/3'>
        <button onClick={handleSignup} className='mt-6 px-6 w-32 py-2 border rounded-xl bg-orange-400 active:bg-orange-700 text-primary-white'>Sign up</button>
        <button onClick={handleLogin} className='mt-6 px-6 w-32 py-2 border rounded-xl bg-slate-400 active:bg-slate-700 text-primary-white'>Login</button>
      </div>
    </div>
  )
}

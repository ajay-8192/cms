    ...
    <div className='w-full overflow-auto scrollbar-hide p-4'>
      <div className='flex justify-between px-4 pt-4'>
        <div className='border-b shadow w-80 rounded'>
          <div className="bg-gray-200 p-2 text-center truncate w-full" onClick={() => setIsOpen(!isOpen)}>
            {selectedOption.name}
          </div>
          {isOpen && (
            <div className="absolute mt-1 max-w-60 bg-white rounded shadow-lg z-20">
              {contentItems.map((option) => (
                <div key={option.id} className="p-2 hover:bg-gray-100 truncate" onClick={() => handleOptionClick(option)}>
                  {option.name}
                </div>
              ))}
            </div>
        ...
        <button className='border px-4 rounded-xl bg-green-500 active:bg-green-700 text-primary-white'>Add new Content</button>
      </div>
      <div className='mt-8 mx-auto p-4'>
        {Object.keys(selectedOption.values).map(item => {
          return (
            <div key={item} className='flex justify-center w-full gap-3 mb-4 rounded-lg'>
              <div className='w-1/4 opacity-60 border px-2'>{item}</div> : 
              <div className='w-3/4 font-medium border px-2'>{selectedOption.values[item as keyof typeof selectedOption.values]}</div>
            </div>
          )
        })}
      </div>
      <div className='my-8 ml-4'>
        <button className='border px-4 py-2 rounded-lg text-white bg-primary active:bg-primary-blue'>Add row</button>
      </div>
    </div>
  )
}
